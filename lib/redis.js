...
function createClient(config, app) {
  ...
  if (config.cluster === true) {
    ...
    if(config.sentinels) {
      assert(config.sentinels && config.sentinels.length > 0 && config.name !== undefined && config.password !== undefined, '[egg-redis] sentinels configuration is required when use sentinels mode');
      config.sentinels.forEach(client=>{
        assert(client.host && client.port, `[egg-redis] host: ${client.host}, prot: ${client.port} are required`);
      });
      app.coreLogger.info('[egg-redis] sentinels connecting start');
      client = new Redis(config);
      client.on('connect', function() {
        app.coreLogger.info('[egg-redis] sentinels connect success');
      });
    } else {
