...
function createClient(config, app) {
  ...
  if (config.cluster === true) {
    ...
    if(config.sentinels) {
      assert(config.sentinels && config.sentinels.length > 0 && name !== undefined && password !== undefined, '[egg-redis] sentinels configuration is required when use sentinels mode');
      config.sentinels.forEach(client=>{
        assert(client.host && client.port, `[egg-redis] host: ${client.host}, prot: ${client.port}`);
      });
